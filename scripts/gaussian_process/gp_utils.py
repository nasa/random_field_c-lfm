...
def get_logging_path_and_name(root_dir, params_dict):
    problem_keys = ["N_data", "gp_var", "gp_cov_len", "use_cov"]
    nn_keys = [
        "latent_dim",
        "h_encoder",
        "h_trunk",
    ...
    problem_dir = "-".join(f"{k}={params_dict[k]}" for k in problem_keys)
    nn_dir = "-".join(f"{k}={params_dict[k]}" for k in nn_keys)
    run_name = "-".join(f"{k}={params_dict[k]}" for k in run_keys)
    path = "/".join([problem_dir, nn_dir])
    path = Path(f"{root_dir}/{path}")
